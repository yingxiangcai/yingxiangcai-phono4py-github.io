...
def calc_dists(sposcar):
    ...
    for j,(ja,jb,jc) in enumerate(itertools.product(range(-1,2),
                                                    range(-1,2),
                                                    range(-1,2))):
...
def write_indexcell(ngrid,poscar,sposcar,dmin,nequi,shifts,frange,filename):
    ...
    shifts27=list(itertools.product(range(-1,2),
                                    range(-1,2),
                                    range(-1,2)))
    ...
    nblocks=0
    f=io.StringIO()
    for ii,jj in itertools.product(range(natoms),
                                   range(ntot)):
    ...
    for ii,jj in itertools.product(range(natoms),
        ...
        for kk in range(ntot):
            ...
            for ll in range(ntot):
                ...
                for mm,nn,oo,pp in itertools.product(range(3),
                                              range(3),
                                              range(3),
                                              range(3)):
