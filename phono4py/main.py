...
class Phono4py:
    ...
    def __init__(self, unitcell, supercell_matrix, primitive_matrix="auto",
        ...
        # Symmetry analyzer - phonopy 4.x uses properties not getters
        self.symmetry = None
    ...
    def run(self, include_3ph=True, include_4ph=True,
        ...
        # Run harmonic mesh - phonopy 4.x uses ph.mesh property
        if self.rank == 0:
            print("Running harmonic calculation...")
        harmonic = HarmonicCalculator(self.phonopy)
        mesh_data = harmonic.run_mesh(self.mesh)

        freqs = mesh_data.frequencies
        eigenvectors = mesh_data.eigenvectors
        group_velocities = mesh_data.group_velocities

        nq = len(qpoints)

        # phonopy 4.x eigenvectors shape: (nq, nband, nband)
        # Reshape to (nq, nband, natom, 3)
        if eigenvectors.ndim == 3 and eigenvectors.shape[2] == self.nband:
            eigenvectors = eigenvectors.reshape(nq, self.nband, self.natom, 3)
        elif eigenvectors.shape != (nq, self.nband, self.natom, 3):
            raise ValueError(f"Unexpected eigenvectors shape: {eigenvectors.shape}")
    ...
    def _build_fc2_full(self):
        """Build full fc2 matrix from sparse data with proper supercell mapping."""
        ...
        # Map from supercell to primitive using p2s_map
        p2s_map = self.phonopy.primitive.p2s_map
        fc2_primitive = np.zeros((self.natom, self.natom, 3, 3))
        for i, p_i in enumerate(p2s_map):
            for j, p_j in enumerate(p2s_map):
                fc2_primitive[i, j] = fc2_full[p_i, p_j]

        return fc2_primitive
