...
import numpy as np
from .utils import mode_heat_capacity, get_mpi_rank, get_mpi_size, mpi_allreduce, parallel_split

class BTEIterativeSolver:
    def __init__(self, primitive, mesh, temperatures, max_iter=50, tol=1e-5, include_4ph_in_iteration=False):
    ...
    def solve(self, freqs, group_velocities, gamma_total, interaction, qpoints, eigenvectors, gamma_3ph=None, gamma_4ph=None):
        ...
        for T in self.temperatures:
            ...
            if gamma_3ph is not None and self.max_iter > 0:
                f = self._iterate_3ph(f, b, Gamma, freqs, eigenvectors, qpoints, interaction, T)
    ...
    def _iterate_3ph(self, f0, b, Gamma, freqs, eigenvectors, qpoints, interaction, T):
        nq, nband = freqs.shape
        f = f0.copy()
                    ...
                    for iq2 in range(nq):
                        ...
                        for ib2 in range(nband):
                            ...
                            for ib3 in range(nband):
                                ...
                                n3 = n_occ[iq3, ib3]
                                v3 = interaction.compute_v3_vectorized(q, q2, q3, e, e2, e3)
                                v3_sq = np.abs(v3)**2
                                if v3_sq < 1e-30:
                                    continue
                                delta_e = omega - omega2 - omega3
                                sigma = adaptive_sigma(omega, omega2, omega3, scalebroad=1.0, dq=1.0/np.mean(self.mesh))
                                delta = gaussian_smearing(delta_e, sigma)
                                occ = n2 + n3 + 1
                                for alpha in range(3):
                                    delta_f[iq, iband, alpha] += v3_sq * occ * delta * (f[iq2, ib2, alpha] + f[iq3, ib3, alpha])
                                delta_e = omega + omega2 - omega3
                                delta = gaussian_smearing(delta_e, sigma)
                                occ = n3 - n2
                                for alpha in range(3):
                                    delta_f[iq, iband, alpha] += v3_sq * occ * delta * (f[iq3, ib3, alpha] - f[iq2, ib2, alpha])
