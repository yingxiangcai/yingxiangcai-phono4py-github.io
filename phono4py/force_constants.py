...
File formats follow the ShengBTE/FourPhonon conventions.
"""
...
def read_force_constants_2nd(filename: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Read second-order force constants (Phonopy format)."""
...
def read_force_constants_3rd(filename: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    return (np.array(atom_triplets),
            np.array(cell_vectors_2),
            np.array(cell_vectors_3),
            np.array(force_constants_list))


def read_force_constants_4th(filename: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Read fourth-order force constants (FourPhonon sparse format)."""
    with open(filename, 'r') as f:
        lines = f.readlines()
    ...
    return (np.array(atom_quartets),
            np.array(cell_vectors_2),
            np.array(cell_vectors_3),
            np.array(cell_vectors_4),
            np.array(force_constants_list))
