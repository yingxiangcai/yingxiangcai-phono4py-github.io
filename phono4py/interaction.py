...
class PhononInteraction:
    ...
    def compute_v3_vectorized(self, q1, q2, q3, e1, e2, e3):
        ...
        phases = np.exp(1j * (np.dot(self.fc3_cells2, q2_cart) + np.dot(self.fc3_cells3, q3_cart)))
        ...
        contractions = np.einsum('nabc,na,nb,nc->n', self.fc3, np.conj(e1_sel), e2_sel, e3_sel)
    ...
    def compute_v4_vectorized(self, q1, q2, q3, q4, e1, e2, e3, e4):
        ...
        phases = np.exp(1j * (np.dot(self.fc4_cells2, q2_cart) + np.dot(self.fc4_cells3, q3_cart) + np.dot(self.fc4_cells4, q4_cart)))
        ...
        contractions = np.einsum('nabcd,na,nb,nc,nd->n', self.fc4, np.conj(e1_sel), e2_sel, e3_sel, e4_sel)
