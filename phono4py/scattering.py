...
import numpy as np
from .utils import bose_einstein, adaptive_sigma, gaussian_smearing, get_mpi_rank, get_mpi_size, mpi_allreduce, parallel_split
from .interaction import PhononInteraction

class ScatteringCalculator:
    def __init__(self, interaction, mesh, temperatures, scalebroad=1.0, use_ir_mesh=True, mapping=None, grid_points=None):
                ...
                for iq2 in range(nq):
                    ...
                    for ib2 in range(nband):
                        ...
                        for ib3 in range(nband):
                            ...
                            n3 = n_occ[iq3, ib3]
                            v3 = self.interaction.compute_v3_vectorized(q, q2, q3, e, e2, e3)
                            v3_sq = np.abs(v3)**2
                            if v3_sq < 1e-30:
                                continue
                            # emission
                            delta_e = omega - omega2 - omega3
                            sigma = adaptive_sigma(omega, omega2, omega3, scalebroad=self.scalebroad, dq=np.mean(self.dq))
            ...
            for iband in range(nband):
                ...
                for iq2 in range(nq):
                    ...
                    for iq3 in range(nq):
                        ...
                        for ib2 in range(nband):
                            omega2 = freqs[iq2, ib2]
                            if omega2 < omega_cutoff: continue
                            e2 = eigenvectors[iq2, ib2]
                            n2 = n_occ[iq2, ib2]
                            for ib3 in range(nband):
                                omega3 = freqs[iq3, ib3]
                                if omega3 < omega_cutoff: continue
                                e3 = eigenvectors[iq3, ib3]
                                n3 = n_occ[iq3, ib3]
                                for ib4 in range(nband):
                                    omega4 = freqs[iq4, ib4]
                                    if omega4 < omega_cutoff: continue
                                    e4 = eigenvectors[iq4, ib4]
                                    n4 = n_occ[iq4, ib4]
                                    v4 = self.interaction.compute_v4_vectorized(q, q2, q3, q4, e, e2, e3, e4)
                                    v4_sq = np.abs(v4)**2
                                    if v4_sq < 1e-30: continue
                                    delta_e = omega + omega2 - omega3 - omega4
                                    sigma = adaptive_sigma(omega, omega2, omega3, omega4, scalebroad=self.scalebroad, dq=np.mean(self.dq))
                                    delta = gaussian_smearing(delta_e, sigma)
                                    n1 = n_occ[iq, iband]
                                    occ = (n3 * n4 * (n1 + 1) * (n2 + 1) - n1 * n2 * (n3 + 1) * (n4 + 1))
                        ...
                        for ib2 in range(nband):
                            ...
                            for ib3 in range(nband):
                                ...
                                for ib4 in range(nband):
                                    ...
                                    delta = gaussian_smearing(delta_e, sigma)
                                    n1 = n_occ[iq, iband]
                                    occ = ((n2 + 1) * (n3 + 1) * (n4 + 1) * n1 - n2 * n3 * n4 * (n1 + 1))
                                    gamma_mode += v4_sq * np.abs(occ) * delta
                gamma[iq, iband] = gamma_mode * prefactor
        gamma = mpi_allreduce(gamma, op='sum')
        return gamma
    ...
    def calculate_total_scattering(self, freqs, eigenvectors, qpoints, include_3ph=True, include_4ph=True):
        results = {}
